...
//   const [type, setType] = useState<'meditation' | 'exercise' | 'mindfulness' | 'gratitude' | 'mood'>('meditation');
...
//         const questsRef = collection(firestore, 'users', user.uid, 'dailyQuests');
//         const questsSnapshot = await getDocs(questsRef);

//         let allActiveQuests: Quest[] = [];
//         let allCompletedQuests: Quest[] = [];

//         questsSnapshot.forEach(doc => {
//           const data = doc.data();
//           if (data.quests) {
//             allActiveQuests.push(...data.quests.filter((q: Quest) => !q.completed));
//             allCompletedQuests.push(...data.quests.filter((q: Quest) => q.completed));
//           }
//         });

//         if (allActiveQuests.length === 0 && allCompletedQuests.length === 0) {
//           const dailyQuestRef = doc(firestore, 'users', user.uid, 'dailyQuests', today);
//           await setDoc(dailyQuestRef, { date: today, quests: defaultQuests });
//           allActiveQuests = [...defaultQuests];
//         }

//         setQuests(allActiveQuests);
...
//     const questDoc = await getDoc(dailyQuestRef);
//     if (questDoc.exists()) {
//       const existingQuests = questDoc.data().quests || [];
//       await updateDoc(dailyQuestRef, {
//         quests: [...existingQuests, newQuest],
//       });
//     } else {
...
//           <TouchableOpacity style={styles.createButton} onPress={handleCreateQuest}>
//             <Text style={{ color: '#fff' }}>Create</Text>
//           </TouchableOpacity>
//           <TouchableOpacity style={styles.cancelButton} onPress={() => setShowModal(false)}>
//             <Text style={{ color: '#4C66EE' }}>Cancel</Text>
//           </TouchableOpacity>
//         </View>
//       </Modal>
//     </SafeAreaView>
...
//   filterButton: { padding: 10, backgroundColor: '#EFF1FF', borderRadius: 8 },
//   tabContainer: { flexDirection: 'row', backgroundColor: '#FFF', paddingHorizontal: 20 },
//   tab: { padding: 10, borderRadius: 20, backgroundColor: '#F0F2F8', marginRight: 10 },
//   activeTab: { backgroundColor: '#4C66EE' },
//   tabText: { color: '#666' },
//   activeTabText: { color: '#FFF' },
//   questList: { padding: 20 },
...
// // // Updated QuestsScreen.tsx with filtering, dynamic quest types, and calendar support
